...
module.exports = function () {
    ...
    this.Then(/^I print response body$/, function (callback) {
        ...
        let body = this.apickli.httpResponse;
        if (typeof body.error !== 'undefined' && typeof body.error.detailedMessage !== 'undefined') {
             body.error.detailedMessage = "";
        }
        console.log(body);
        callback();
    });
    ...
    this.When(/^I set basic token$/, function (callback) {
        this.apickli.addRequestHeader("x-client-auth", "Basic TUNJREE7KIMwcjEkNjFrdXRjdzMkOXpxYkL9IP8vUUE5TGE5NWRiSGJGLw==");
        callback();
    });

    this.When(/^I set header APIVersion$/, function (callback) {
        this.apickli.addRequestHeader("X-Api-Version", APIVersion);
        callback();
    });

    this.When(/^I set header X-Metadata$/, function (callback) {
        this.apickli.addRequestHeader("X-Metadata", XMetadata);
        callback();
    });
    ...
    this.When(/^I set bearer token$/, function (callback) {
        this.apickli.addRequestHeader('X-Client-Auth', 'Bearer '+this.apickli.getGlobalVariable("access_token"));
        callback();
    });
