...
declare function xf:CriaNotificacaoOutageJPDV1_0outageIntegracaoWAServiceControlnotifyOutageRequest($parametros as element(ns0:ParametrosNotificarOutage))
    as element(ns-1:notifyOutageRequest) {
        <ns-1:notifyOutageRequest>
            ...
            {
                for $pontoComum in $parametros/ns0:pontoComum
                return
                    <ns-1:pontoComum>
                            ...
                            return
                                <ns-1:node>
                                    ...
                                    {                                      
                                        ...
                                        return
                                            <ns-1:celula>
                                                <ns-1:cellId>{ data($celula/ns0:cellId) }</ns-1:cellId>
                                                
                                                {
                                                    for $logradouro in $celula/ns0:logradouro
                                                     return
                                                       <ns-1:logradouro>
                                                          <ns-1:logradouroId>{ data($logradouro/ns0:logradouroId) }</ns-1:logradouroId>
                                                          {
                                                             for $imovel in $logradouro/ns0:imovel
                                                                return
                                                                  <ns-1:imovel>
                                                                    <ns-1:imovelId>{ data($imovel/ns0:imovelId) }</ns-1:imovelId>
                                                                    {
                                                                       for $hp in $imovel/ns0:hp
                                                                        return
                                                                          <ns-1:hp>
                                                                            <ns-1:hpId>{ data($hp/ns0:hpId) }</ns-1:hpId>
                                                                          </ns-1:hp>
                                                                     }
                                                                 </ns-1:imovel>                                                          
                                                          }
...
declare variable $docDadosEnder as element(ns0:endereco) external;
